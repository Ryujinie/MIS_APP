    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/hr_overtime.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/hr_overtime.pdf');
        } else {
...
class PrintHrOvertime extends StatefulWidget {
  const PrintHrOvertime({super.key});

  @override
  State<PrintHrOvertime> createState() => _PrintHrOvertimeState();
}

class _PrintHrOvertimeState extends State<PrintHrOvertime> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/hr_overtime.pdf');
  }
