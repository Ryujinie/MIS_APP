    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/hr_profile.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/hr_profile.pdf');
        } else {
...
class PrintHRProfile extends StatefulWidget {
  const PrintHRProfile({super.key});

  @override
  State<PrintHRProfile> createState() => _PrintHRProfileState();
}

class _PrintHRProfileState extends State<PrintHRProfile> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/hr_profile.pdf');
  }
                ...
                child: Column(
                  children: [
                    Container(
                      ...
                      decoration:
                          const BoxDecoration(color: constants.hrPrimary),
                      child: Row(
                        children: [
