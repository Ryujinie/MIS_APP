    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/hr_attendance.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/hr_attendance.pdf');
        } else {
...
class PrintHrAttendance extends StatefulWidget {
  const PrintHrAttendance({super.key});

  @override
  State<PrintHrAttendance> createState() => _PrintHrAttendanceState();
}

class _PrintHrAttendanceState extends State<PrintHrAttendance> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/hr_attendance.pdf');
  }
