    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/hr_leave.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/hr_leave.pdf');
        } else {
...
class PrintHrLeave extends StatefulWidget {
  const PrintHrLeave({super.key});

  @override
  State<PrintHrLeave> createState() => _PrintHrLeaveState();
}

class _PrintHrLeaveState extends State<PrintHrLeave> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/hr_leave.pdf');
  }
