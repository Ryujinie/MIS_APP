    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/admin_leave.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/admin_leave.pdf');
        } else {
...
class PrintAdminLeave extends StatefulWidget {
  const PrintAdminLeave({super.key});

  @override
  State<PrintAdminLeave> createState() => _PrintAdminLeaveState();
}

class _PrintAdminLeaveState extends State<PrintAdminLeave> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/admin_leave.pdf');
  }
