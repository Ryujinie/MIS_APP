    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/admin_emp.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/admin_emp.pdf');
        } else {
...
class PrintAdminEmpMngmt extends StatefulWidget {
  const PrintAdminEmpMngmt({super.key});

  @override
  State<PrintAdminEmpMngmt> createState() => _PrintAdminEmpMngmtState();
}

class _PrintAdminEmpMngmtState extends State<PrintAdminEmpMngmt> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/admin_emp.pdf');
  }
