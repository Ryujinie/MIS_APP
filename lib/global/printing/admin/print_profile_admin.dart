    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/admin_profile.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/admin_profile.pdf');
        } else {
...
class PrintAdminProfile extends StatefulWidget {
  const PrintAdminProfile({super.key});

  @override
  State<PrintAdminProfile> createState() => _PrintAdminProfileState();
}

class _PrintAdminProfileState extends State<PrintAdminProfile> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/admin_profile.pdf');
  }
