    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/manager_overtime.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/manager_overtime.pdf');
        } else {
...
class PrintManagerOvertime extends StatefulWidget {
  const PrintManagerOvertime({super.key});

  @override
  State<PrintManagerOvertime> createState() => _PrintManagerOvertimeState();
}

class _PrintManagerOvertimeState extends State<PrintManagerOvertime> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/manager_overtime.pdf');
  }
