    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/manager_attendance.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/manager_attendance.pdf');
        } else {
...
class PrintManagerAttendance extends StatefulWidget {
  const PrintManagerAttendance({super.key});

  @override
  State<PrintManagerAttendance> createState() => _PrintManagerAttendanceState();
}

class _PrintManagerAttendanceState extends State<PrintManagerAttendance> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/manager_attendance.pdf');
  }
