    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/supervisor_leave.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/supervisor_leave.pdf');
        } else {
...
class PrintSupervisorLeave extends StatefulWidget {
  const PrintSupervisorLeave({super.key});

  @override
  State<PrintSupervisorLeave> createState() => _PrintSupervisorLeaveState();
}

class _PrintSupervisorLeaveState extends State<PrintSupervisorLeave> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/supervisor_leave.pdf');
  }
