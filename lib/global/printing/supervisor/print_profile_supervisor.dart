    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/supervisor_profile.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/supervisor_profile.pdf');
        } else {
...
class PrintSupervisorProfile extends StatefulWidget {
  const PrintSupervisorProfile({super.key});

  @override
  State<PrintSupervisorProfile> createState() => _PrintSupervisorProfileState();
}

class _PrintSupervisorProfileState extends State<PrintSupervisorProfile> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/supervisor_profile.pdf');
  }
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(
                            ...
                            width: 120,
                            child:
                                Image.asset('assets/images/admin_mis_logo.png'),
                          ),
                          const Spacer(),
                          Text(
                            'Profile Information',
