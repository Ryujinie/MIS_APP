    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/supervisor_overtime.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/supervisor_overtime.pdf');
        } else {
...
class PrintSupervisorOvertime extends StatefulWidget {
  const PrintSupervisorOvertime({super.key});

  @override
  State<PrintSupervisorOvertime> createState() =>
      _PrintSupervisorOvertimeState();
}

class _PrintSupervisorOvertimeState extends State<PrintSupervisorOvertime> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/supervisor_overtime.pdf');
  }
