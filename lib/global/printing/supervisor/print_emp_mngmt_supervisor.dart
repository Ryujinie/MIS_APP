    ...
    if (boundary != null) {
      ...
      if (byteData != null) {
        ...
        if (documentsPath != null && documentsPath.isNotEmpty) {
          final pdfFile = File('$documentsPath/supervisor_emp.pdf');
          await compute(
              convertImageToPdf
                  as ComputeCallback<Map<String, Object>, dynamic>,
              {'imageBytes': imageBytes, 'pdfFilePath': pdfFile.path});

          print('PDF saved to $documentsPath/supervisor_emp.pdf');
        } else {
...
class PrintSupervisorEmpMngmt extends StatefulWidget {
  const PrintSupervisorEmpMngmt({super.key});

  @override
  State<PrintSupervisorEmpMngmt> createState() =>
      _PrintSupervisorEmpMngmtState();
}

class _PrintSupervisorEmpMngmtState extends State<PrintSupervisorEmpMngmt> {
  ...
  void _onOpenButtonPressed() async {
    final documentsPath = await getDocumentsPath();
    OpenFile.open('$documentsPath/supervisor_emp.pdf');
  }
